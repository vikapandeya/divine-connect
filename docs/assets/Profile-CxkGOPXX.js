import{r as D,j as e}from"./vendor-BLqV_m0t.js";import{f as h,D as ne,e as ae,h as ye,i as ve,j as Ne,k as je,m as we,n as Ce,o as De}from"./index-BTJX4UBJ.js";import{n as $e,P as ke,o as Re,m as M,p as P,S as ie,q as le,R as Se,D as w,r as E,A as ce,s as Ae}from"./icons-dZXZzUTT.js";import{u as Ie,c as Te}from"./router-BZJXDFZY.js";import"./motion-D5xlclFS.js";const B=595,H=842;function ze(t){return t.replace(/\\/g,"\\\\").replace(/\(/g,"\\(").replace(/\)/g,"\\)")}function Pe(t,s){const a=t.split(/\s+/).filter(Boolean);if(a.length===0)return[""];const r=[];let n="";return a.forEach(c=>{const i=n?`${n} ${c}`:c;if(i.length<=s){n=i;return}n&&r.push(n),n=c}),n&&r.push(n),r}function S(t=[.16,.13,.11]){return t.map(s=>Number(s.toFixed(3)))}function Ee(t,s){return Math.max(1,t.length)*s*.52}function Oe(t,s,a){const r=t.trim();if(!r)return[""];const n=r.split(/\n+/),c=s?Math.max(10,Math.floor(s/Math.max(4,a*.52))):Math.max(10,Math.floor(460/Math.max(4,a*.52)));return n.flatMap((i,l)=>{const g=Pe(i,c);return l===0?g:["",...g]})}function Le(t){const s=[],a=t.width??B,r=t.height??H;if(t.backgroundColor){const[n,c,i]=S(t.backgroundColor);s.push(`${n} ${c} ${i} rg`),s.push(`0 0 ${a} ${r} re f`)}return t.elements.forEach(n=>{if(n.type==="rect"){if(s.push("q"),n.fillColor){const[b,x,j]=S(n.fillColor);s.push(`${b} ${x} ${j} rg`)}if(n.strokeColor){const[b,x,j]=S(n.strokeColor);s.push(`${b} ${x} ${j} RG`),s.push(`${n.strokeWidth??1} w`)}s.push(`${n.x} ${n.y} ${n.width} ${n.height} re`),n.fillColor&&n.strokeColor?s.push("B"):n.fillColor?s.push("f"):n.strokeColor&&s.push("S"),s.push("Q");return}if(n.type==="line"){const[b,x,j]=S(n.color??[.8,.76,.7]);s.push("q"),s.push(`${b} ${x} ${j} RG`),s.push(`${n.width??1} w`),s.push(`${n.x1} ${n.y1} m ${n.x2} ${n.y2} l S`),s.push("Q");return}const c=n.size??12,i=n.lineHeight??c+5,[l,g,u]=S(n.color??[.16,.13,.11]),v=Oe(n.text,n.maxWidth,c);s.push("BT"),s.push(`${l} ${g} ${u} rg`),v.forEach((b,x)=>{const j=n.bold?"F2":"F1",m=Ee(b,c);let R=n.x;n.align==="center"?R=n.x-m/2:n.align==="right"&&(R=n.x-m),s.push(`/${j} ${c} Tf`),s.push(`1 0 0 1 ${R.toFixed(2)} ${(n.y-x*i).toFixed(2)} Tm`),s.push(`(${ze(b)}) Tj`)}),s.push("ET")}),s.join(`
...
`;const c=[0];r.forEach(l=>{c.push(n.length),n+=`${l}
