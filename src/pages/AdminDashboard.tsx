...
export default function AdminDashboard() {
  ...
  const fetchWhatsAppBookings = async () => {
    try {
      const response = await fetch('/api/whatsapp-bookings');
      if (response.ok) {
        setWhatsappBookings(await response.json());
      }
    } catch (error) {
      console.error('Error fetching WhatsApp bookings:', error);
    }
  };
  ...
  const fetchPayouts = async () => {
    try {
      ...
      if (response.ok) {
        setPayouts(await response.json());
      }
    } catch (error) {
  ...
  const handleUpdateWhatsAppStatus = async (id: string, status: 'approved' | 'rejected') => {
    if (!window.confirm(`Are you sure you want to ${status} this booking?`)) return;
    try {
      const response = await fetch(`/api/whatsapp-bookings/${id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('Error updating WhatsApp booking status:', error);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex bg-stone-100 p-1 rounded-2xl mr-4">
            ...
            >
              ...
              {whatsappBookings.filter(b => b.status === 'pending').length > 0 && (
                <span className="ml-2 px-2 py-0.5 bg-emerald-500 text-white text-[10px] rounded-full">
                  {whatsappBookings.filter(b => b.status === 'pending').length}
                </span>
              )}
            </button>
            <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm">
                <h3 className="text-xl font-serif font-bold text-stone-900">WhatsApp Booking Requests</h3>
              </div>

              {whatsappBookings.length === 0 ? (
                <div className="bg-white p-12 rounded-3xl border border-stone-200 text-center shadow-sm">
                  ...
                  <p className="text-stone-500">No WhatsApp booking requests found.</p>
                </div>
              ) : (
                <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-sm">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-stone-50 border-bottom border-stone-200">
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Puja</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">User WhatsApp</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Distance</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Status</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-stone-100">
                        {whatsappBookings.map((booking) => (
                          <tr key={booking.id} className="hover:bg-stone-50 transition-colors">
                            <td className="px-6 py-4">
                              <span className="font-bold text-stone-900">{booking.pujaTitle}</span>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4 text-right">
                              {booking.status === 'pending' && (
                                <div className="flex justify-end space-x-2">
                                  <button 
                                    onClick={() => handleUpdateWhatsAppStatus(booking.id, 'approved')}
                                  ...
                                  <button 
                                    onClick={() => handleUpdateWhatsAppStatus(booking.id, 'rejected')}
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm">
                <h3 className="text-xl font-serif font-bold text-stone-900">Payout Requests</h3>
              </div>

              {payouts.length === 0 ? (
                <div className="bg-white p-12 rounded-3xl border border-stone-200 text-center shadow-sm">
                  <IndianRupee className="w-12 h-12 text-stone-200 mx-auto mb-4" />
                ...
                <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-sm">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-stone-50 border-bottom border-stone-200">
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Vendor ID</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Amount</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Bank Details</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider">Status</th>
                          <th className="px-6 py-4 text-xs font-bold text-stone-400 uppercase tracking-wider text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-stone-100">
                        {payouts.map((payout) => (
                          <tr key={payout.id} className="hover:bg-stone-50 transition-colors">
                            <td className="px-6 py-4">
                              <span className="font-bold text-stone-900">{payout.vendorId}</span>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center font-bold text-orange-600">
                                <IndianRupee className="w-3 h-3 mr-1" />
                            ...
                            <td className="px-6 py-4">
                              <div className="text-xs text-stone-600">
                                {typeof payout.bankDetails === 'string' ? payout.bankDetails : JSON.stringify(payout.bankDetails)}
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
